...
class Voice(Data):
    def __init__(self, voice_id: str, caption: str = None, value_id: int = None):
        super().__init__(voice_id, value_id)
        self.__caption = caption
    ...
    async def save(self, connection: asyncpg.connection.Connection, user_id: uuid.UUID, update_id: int):
        self.value_id = await connection.fetchval('INSERT INTO voices (value, caption) '
                                                  'VALUES ($1, $2) RETURNING voice_id;',
                                                  self.value, self.caption)
        await connection.execute('INSERT INTO updates (type, user_id, update_id, value_id) VALUES ($1, $2, $3, $4);',
                                 'voice', user_id, update_id, self.value_id)

    def __repr__(self):
        return f'Voice({self.value}, {self.caption})'


class File(Data):
    def __init__(self, file_id: str, value_id: int = None):
        super().__init__(file_id, value_id)
...
class Update(ABC):
    ...
    async def find_user(self, json_update: dict, connection: asyncpg.connection.Connection):
        ...
        if await connection.fetchval('SELECT COUNT(*) FROM users WHERE chat_id = $1;', chat_id) == 0:
            ...
            user_info = dict(await connection.fetchrow('SELECT phone, name, surname, role, user_id '
                                                       'FROM users '
                                                       'WHERE chat_id = $1;',
                                                       chat_id))
    ...
    def _get_data(json_update: dict) -> Data:
        ...
        if message_type == 'document':
            ...
            return Voice(voice_id=json_update['message']['voice'][-1]['file_id'],
                         caption=json_update['message'].get('caption'))
        elif message_type == 'file':
            return File(file_id=json_update['message']['file'][-1]['file_id'],
